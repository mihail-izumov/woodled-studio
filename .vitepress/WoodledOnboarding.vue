...
onMounted(() => {
  ...
  if (typeof document !== 'undefined' && !document.getElementById('woodled-fonts')) {
    ...
    l.id = 'woodled-fonts'
    l.rel = 'stylesheet'
    // Roboto Flex — основной текст (variable font 100..1000, нативные
    // веса 400/500/600/700 без синтеза). Old Standard TT — серифные
    // названия глав (.cn). Кириллица идёт по unicode-range Google Fonts.
    l.href = 'https://fonts.googleapis.com/css2?family=Old+Standard+TT:ital,wght@0,400;0,700;1,400&family=Roboto+Flex:wght@100..1000&display=swap'
...
.sh-title { font-size: 22px; font-weight: 700; color: var(--text); text-align: center; margin-top: 28px; animation: fu .6s .2s both; }
.sh-bars {
...
.rb { padding: 4px 14px; border-radius: 12px; border: 1px solid; font-size: 14px; font-weight: 600; transition: all .8s; }
...
.d5zn { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: var(--text); }
.d5zp { font-size: 12px; font-weight: 700; padding: 1px 6px; border-radius: 6px; transition: all .8s; }
...
.d5btn { display: block; width: 100%; padding: 18px 0; background: var(--gold); border: none; border-radius: 12px; color: var(--bg); font-size: 18px; font-weight: 600; cursor: pointer; box-shadow: 0 4px 24px color-mix(in srgb, var(--gold) 20%, transparent); }
