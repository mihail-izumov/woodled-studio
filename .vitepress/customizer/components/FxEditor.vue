...
/* 3D-шар для опций (чаша / температура).
   radial-gradient + inset shadow = «бликующий шар» без SVG. */
...
function sphereStyle(s: SphereStops, size = 36): Record<string, string> {
  return {
    ...
    height: `${size}px`,
    borderRadius: '50%',
    background: `radial-gradient(circle at 32% 28%, ${s.hi} 0%, ${s.mid} 42%, ${s.lo} 100%)`,
    boxShadow: `inset -3px -5px 7px rgba(0,0,0,.32), 0 2px 8px rgba(0,0,0,.35)${s.glow !== 'rgba(0,0,0,0)' ? `, 0 0 16px ${s.glow}` : ''}`,
    flexShrink: '0',
  }
}
          ...
          <div v-if="curStep==='size'&&families">
            ...
            <div :style="{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'14px'}">
              <button v-for="fid in families" :key="fid" :style="{padding:'14px 10px',borderRadius:'10px',cursor:'pointer',textAlign:'center',border:build.m===fid?`2px solid ${T.text}`:`1px solid ${T.border}`,background:build.m===fid?T.text+'10':T.cardAlt,position:'relative',display:'flex',flexDirection:'column',alignItems:'center',gap:'8px'}" @click="()=>{mid=fid;upBuild({m:fid,lamps:MD[fid].lamps})}">
                <div :style="{display:'flex',alignItems:'center',justifyContent:'center',width:'100%',gap:'6px'}"><span :style="{fontSize:'18px',fontWeight:800,color:build.m===fid?T.text:T.textSec,textAlign:'center',lineHeight:1.1}">{{ SIZE_WORD[MD[fid].letter] ?? MD[fid].letter }}</span><span v-if="recommendedMid===fid" :style="{width:'20px',height:'20px',borderRadius:'50%',background:'#39FF85',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'12px',color:T.bg,fontWeight:700,flexShrink:0,boxShadow:'0 0 8px rgba(57,255,133,.55)'}">✓</span></div>
