    ...
    <div :style="{maxWidth:'480px',margin:'0 auto',padding:view==='summary'?'16px 20px 110px':'16px 20px',fontFamily:`'Segoe UI', system-ui, sans-serif`,color:T.text,boxSizing:'border-box'}">
      ...
      <template v-if="view==='summary'">
        ...
        <div :style="{background:T.card,border:`1px solid ${T.border}`,borderRadius:'16px',padding:'16px',marginBottom:'16px'}">
          ...
          <button v-for="(s,i) in steps" :key="s" :style="{display:'flex',alignItems:'center',gap:'12px',width:'100%',padding:'12px 0',background:'none',border:'none',cursor:'pointer',borderBottom:i<steps.length-1?`1px solid ${T.border}`:'none',textAlign:'left',fontFamily:'inherit'}" @click="goToStep(i)">
            ...
            <span :style="{display:'inline-flex',alignItems:'center',justifyContent:'center',gap:'6px',minWidth:'88px',boxSizing:'border-box',fontSize:'12px',padding:'6px 12px',borderRadius:'8px',fontWeight:600,background:build.steps[s]==='chosen'?T.text+'15':'transparent',border:build.steps[s]==='chosen'?'1px solid transparent':`1px solid ${T.border}`,color:build.steps[s]==='chosen'?T.text:T.textSec}">
              <svg v-if="build.steps[s]==='chosen'" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#39FF85" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" :style="{filter:'drop-shadow(0 0 4px rgba(57,255,133,.7))',flexShrink:0}"><polyline points="20 6 9 17 4 12"/></svg>
              <span>{{ build.steps[s]==='chosen'?'Готово':'Выбрать' }}</span>
            </span>
          </button>

          <!-- Тихая ссылка на гид — когда плашки нет и сборка не завершена.
               Расширенная tap-зона: 16px вверху и 14px внизу + горизонтально на всю
               ширину карточки за счёт отрицательных margin'ов (paddingTop карточки=16). -->
          <button v-if="!showGuidedCTA&&!isDone" :style="{display:'flex',alignItems:'center',justifyContent:'center',gap:'10px',width:`calc(100% + 32px)`,margin:'4px -16px -16px',padding:'16px 16px 18px',background:'none',border:'none',borderTop:`1px solid ${T.border}`,cursor:'pointer',fontFamily:'inherit'}" @click="launchGuided">
      ...
      <template v-if="view==='steps'">
        ...
        <div :style="{background:T.card,border:`1px solid ${T.border}`,borderRadius:'12px',padding:'16px'}">
          ...
          <div v-else-if="curStep==='bowl'"><div v-if="freeBowls.length>0"><div :style="{fontSize:'10px',fontWeight:700,color:T.textDim,textTransform:'uppercase',letterSpacing:'.8px',marginBottom:'6px'}">{{ paidBowls.length>0?'Стандарт · входит в цену':'Выберите чашу' }}</div><div :style="{display:'grid',gridTemplateColumns:'repeat(auto-fill, minmax(100px, 1fr))',gap:'6px'}"><button v-for="b in freeBowls" :key="b.id" :style="{padding:'14px 10px',minHeight:'64px',borderRadius:'8px',cursor:'pointer',textAlign:'center',display:'flex',alignItems:'center',justifyContent:'center',border:build.bowl===b.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.bowl===b.id?T.text+'15':T.card,color:T.text}" @click="upBuild({bowl:b.id})"><div :style="{fontSize:'16px',fontWeight:700,color:T.text,lineHeight:1.25}">{{ b.name }}</div></button></div></div><div v-if="paidBowls.length>0" :style="{marginTop:freeBowls.length>0?'12px':'0'}"><div :style="{fontSize:'10px',fontWeight:700,color:T.textSec,textTransform:'uppercase',letterSpacing:'.8px',marginBottom:'6px'}">С доплатой</div><div :style="{display:'grid',gridTemplateColumns:'repeat(auto-fill, minmax(120px, 1fr))',gap:'6px'}"><button v-for="b in paidBowls" :key="b.id" :style="{padding:'12px 10px',minHeight:'78px',borderRadius:'8px',cursor:'pointer',textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',border:build.bowl===b.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.bowl===b.id?T.text+'15':T.card,color:T.text}" @click="upBuild({bowl:b.id})"><div :style="{fontSize:'16px',fontWeight:700,color:T.text,lineHeight:1.25}">{{ b.name }}</div><div :style="{fontSize:'12px',color:T.textSec,marginTop:'4px'}">+{{ fmt(b.price) }} ₽</div></button></div></div></div>
          <div v-else-if="curStep==='temp'" :style="{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'6px'}"><button v-for="bt in BTEMPS" :key="bt.id" :style="{padding:'14px 8px',minHeight:'140px',borderRadius:'8px',cursor:'pointer',textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-start',border:build.btemp===bt.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.btemp===bt.id?T.text+'15':T.card,color:T.text}" @click="upBuild({btemp:bt.id})"><div :style="{fontSize:'16px',fontWeight:700,color:T.text,lineHeight:1.2}">{{ bt.label }}</div><div :style="{fontSize:'12px',color:T.textSec,marginTop:'4px'}">{{ bt.kelvin }}К</div><div v-if="bt.tip" :style="{fontSize:'12px',color:T.textSec,marginTop:'8px',lineHeight:1.4}">{{ bt.tip }}</div></button></div>
          ...
          <div v-else-if="curStep==='wire'&&model.wireOpts" :style="{display:'flex',flexDirection:'column',gap:'6px'}"><button v-for="w in model.wireOpts" :key="w.id" :style="{textAlign:'left',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'12px',padding:'14px 16px',minHeight:'76px',borderRadius:'8px',cursor:'pointer',border:build.wire===w.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.wire===w.id?T.text+'15':T.card,color:T.text}" @click="upBuild({wire:w.id})"><div :style="{flex:1,minWidth:0}"><div :style="{fontSize:'16px',fontWeight:700,color:T.text,lineHeight:1.25}">{{ w.label }}</div><div v-if="w.tip" :style="{fontSize:'12px',color:T.textSec,marginTop:'4px',lineHeight:1.4}">{{ w.tip }}</div></div><span v-if="w.price>0" :style="{fontSize:'13px',fontWeight:700,color:T.textSec,flexShrink:0}">+{{ fmt(w.price) }} ₽</span><span v-else :style="{fontSize:'11px',color:T.textSec,flexShrink:0}">стандарт</span></button></div>
          <div v-else-if="curStep==='base'" :style="{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}"><button v-for="(bc,bcId) in BASE_COLORS" :key="bcId" :style="{padding:'22px 12px',minHeight:'130px',borderRadius:'8px',cursor:'pointer',textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',border:build.baseColor===bcId?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.baseColor===bcId?T.text+'15':T.card,color:T.text}" @click="upBuild({baseColor:String(bcId)})"><div :style="{width:'40px',height:'40px',borderRadius:'50%',background:bc.color,margin:'0 auto 10px',border:`1px solid ${T.border}`}"/><div :style="{fontSize:'17px',fontWeight:700,color:T.text,lineHeight:1.2}">{{ bc.name }}</div></button></div>
          <div v-else-if="curStep==='bulbs'&&model.bulbOpts" :style="{display:'flex',flexDirection:'column',gap:'6px'}"><div :style="{fontSize:'12px',color:T.textSec,marginBottom:'2px'}">Включить в комплект?</div><button v-for="bo in model.bulbOpts" :key="bo.id" :style="{textAlign:'left',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'12px',padding:'16px',minHeight:'68px',borderRadius:'8px',cursor:'pointer',border:build.bulbOpt===bo.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.bulbOpt===bo.id?T.text+'15':T.card,color:T.text}" @click="upBuild({bulbOpt:bo.id})"><span :style="{fontSize:'16px',fontWeight:700,color:T.text,lineHeight:1.25,flex:1,minWidth:0}">{{ bo.label }}</span><span v-if="bo.price>0" :style="{fontSize:'13px',fontWeight:700,color:T.textSec,flexShrink:0}">+{{ fmt(bo.price) }} ₽</span></button></div>
          <div v-else-if="curStep==='bulbs'" :style="{display:'flex',flexDirection:'column',gap:'8px'}"><button :style="{width:'100%',padding:'16px',minHeight:'92px',borderRadius:'10px',cursor:'pointer',textAlign:'left',border:build.bulbs?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.bulbs?T.text+'12':T.cardAlt,color:T.text,display:'flex',alignItems:'center',justifyContent:'space-between',gap:'12px'}" @click="upBuild({bulbs:true})"><div :style="{flex:1,minWidth:0}"><div :style="{fontSize:'16px',fontWeight:700,color:T.text,lineHeight:1.25}">Да, {{ build.lamps }} {{ slw(build.lamps) }} в комплекте</div><div :style="{fontSize:'12px',color:T.textSec,marginTop:'6px',lineHeight:1.4}">{{ build.lamps }} {{ spw(build.lamps) }} × {{ fmt(bulbPer()) }} ₽</div></div><span :style="{fontSize:'14px',fontWeight:700,color:T.textSec,flexShrink:0}">+{{ fmt(bulbTotal()) }} ₽</span></button><button :style="{width:'100%',padding:'16px',minHeight:'62px',borderRadius:'10px',cursor:'pointer',textAlign:'left',border:!build.bulbs?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:!build.bulbs?T.text+'12':T.cardAlt,color:T.textSec,fontSize:'14px',lineHeight:1.4}" @click="upBuild({bulbs:false})">Подберу свои лампочки</button></div>
        </div>
