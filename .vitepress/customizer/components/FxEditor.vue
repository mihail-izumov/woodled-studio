...
import { byModel, byFixture, toDisplayItem, preloadAspects, seedInteriorsForBuild, preloadSeedAspects } from '../engine/gallery-engine'
...
//
// Иерархия:
//   1. byFixture(model + wood) — STRICT-курация из gallery.ts под выбор юзера.
//      Юзер выбрал чёрный дуб → видит только чёрный (fix bug: «фильтр не работает»).
//   2. Если курация пуста → подкачка из _seed.js (strict по дереву).
//   3. Если и то и то пусто → relaxed-wood seed fallback (показать хоть что-то).
//
// Принцип «куратированные главные» (по правилу продакта): seed добавляется
// ТОЛЬКО когда курация для текущего fixture+wood пуста. Иначе курация
// единственный источник — это убирает визуальные дубликаты (unit/oak).

const galleryItems = computed(() => byFixture({ m: mid.value, wood: build.value.wood }))
...
const seedInteriorDisplayItems = computed(() => {
  // Курация уже есть → seed не нужен.
  if (curatedDisplayItems.value.length > 0) return []
  // Курация пуста → strict-seed под текущее дерево.
  const strict = seedInteriorsForBuild(build.value, 0)
  if (strict.length > 0) return strict
  // И там пусто → relaxed-wood (показываем другое дерево, лучше чем пустота).
  return seedInteriorsForBuild(build.value, 0, { relaxWood: true })
})
watch(seedInteriorDisplayItems, items => {
  if (items.length) preloadSeedAspects(items.map(i => i.src))
}, { immediate: true })

// Финальный список для GallerySection.
...
const sizeAdvice=computed<SizeAdvice|null>(()=>{
  ...
  const comfCount=recs.filter(r=>r.projectedRatio>=0.8&&r.projectedRatio<=1.2).length
  ...
  if(uniqueCats.length===1){if(selBright==='Темно'||selBright==='Полусвет')return{text:`Ни один размер этой коллекции не осветит ${props.roomArea} м² самостоятельно. Выберите крупнейший — он даст максимум, а остальной свет добавьте бра, торшером или спотами.`,tone:'bad'};if(selBright==='Праздник')return{text:`Все размеры дают слишком много света для этой комнаты. Возьмите наименьший и обязательно поставьте диммер — будете управлять яркостью.`,tone:'warn'};if(selBright==='Ярко')return{text:`Все размеры дают запас яркости. Крупный — для выразительного потолка, компактный — если хотите минимализм. Диммер пригодится в любом случае.`,tone:'neutral'};return{text:`Все размеры подходят по яркости — свет одинаковый. Крупнее — выразительнее на потолке, компактнее — деликатнее. Выбирайте по диаметру под высоту потолка и стиль.`,tone:'neutral'}}
  if(rec&&sel){const recName=fxNav(rec.mid);if(sel.modelLm<rec.modelLm){if(selBright==='Темно')return{text:`Этого размера не хватит — будет темно. Рекомендуем ${recName} или добавьте дополнительные светильники (бра, торшер).`,tone:'bad'};if(selBright==='Полусвет')return{text:`Получится приглушённый свет — для атмосферы хорошо, для работы мало. Хотите ярче — возьмите ${recName}.`,tone:'warn'};return{text:`Чуть меньше рекомендуемого, но всё равно комфортно. Если любите яркость — возьмите ${recName}.`,tone:'neutral'}};if(selBright==='Праздник')return{text:`Этот размер даст слишком много света. Возьмите ${recName} — он идеально балансирует, или поставьте диммер.`,tone:'warn'};if(selBright==='Ярко')return{text:`Будет с запасом — хорошо для рабочей зоны или если в комнате тёмные стены. Поставьте диммер для вечернего режима.`,tone:'neutral'};return{text:`Хороший выбор. Рекомендация — ${recName}, но ваш вариант тоже в комфортном диапазоне.`,tone:'good'}}
  if(selBright==='Темно')return{text:`Для ${props.roomArea} м² этого мало. Возьмите крупнее или добавьте другие светильники — бра, торшер, споты.`,tone:'bad'};if(selBright==='Полусвет')return{text:`Приглушённый свет — создаст атмосферу, но для постоянного использования мало. Добавьте бра или возьмите размер крупнее.`,tone:'warn'};if(selBright==='Праздник')return{text:`Много света для этой комнаты. Возьмите размер поменьше или установите диммер.`,tone:'warn'};if(selBright==='Ярко')return{text:`С запасом яркости — комфортно днём, а вечером приглушите диммером. Хороший вариант для светлых комнат и рабочих зон.`,tone:'neutral'};return{text:`Комфортный уровень света для вашей комнаты.`,tone:'good'}
})
function adviceColor(tone:'good'|'warn'|'bad'|'neutral'):string{if(tone==='good')return T.green;if(tone==='warn')return T.textSec;if(tone==='bad')return T.red;return T.neutral}
    ...
    <div :style="{maxWidth:'480px',margin:'0 auto',padding:view==='summary'?'16px 20px 110px':'16px 20px',fontFamily:`'Segoe UI', system-ui, sans-serif`,color:T.text,boxSizing:'border-box'}">
      ...
      <template v-if="view==='summary'">
        ...
        <div ref="titleEl" :style="{marginBottom:'18px',textAlign:'center'}">
          <span :style="{display:'inline-block',padding:'7px 16px',borderRadius:'22px',background:(props.roomTint||T.neutral)+'1A',border:`1px solid ${(props.roomTint||T.neutral)}44`,fontSize:'17px',fontWeight:500,color:props.roomTint||T.neutral,textTransform:'uppercase',letterSpacing:'1.9px',marginBottom:'14px',lineHeight:1}">{{ fxLine(build.m) }}</span>
          <h1 :style="{margin:0,fontSize:'34px',fontWeight:600,color:T.text,lineHeight:1.05,letterSpacing:'-.015em',textAlign:'center'}">{{ fxTitle(build.m) }}</h1>
        </div>
        ...
        <div :style="{background:T.card,border:`1px solid ${T.border}`,borderRadius:'12px',padding:'16px'}">
          <div v-if="curStep==='size'&&families">
            <div v-if="hasRoomContext" :style="{textAlign:'center',marginBottom:'14px'}">
              ...
              <button :style="{display:'inline-flex',alignItems:'center',gap:'6px',padding:'6px 14px',borderRadius:'14px',background:T.text+'15',border:`1px solid ${T.text}44`,color:T.text,cursor:'pointer',fontSize:'12px',fontWeight:500}" @click="showHelp=true"><span :style="{width:'16px',height:'16px',borderRadius:'50%',background:T.text,color:T.bg,display:'inline-flex',alignItems:'center',justifyContent:'center',fontSize:'10px',fontWeight:800}">?</span><span>Как подбирается размер</span></button>
            </div>
            <div :style="{fontSize:'13px',fontWeight:600,color:T.textSec,marginBottom:'10px'}">Сравните размеры:</div>
            <div :style="{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'14px'}">
              <button v-for="fid in families" :key="fid" :style="{padding:'14px 10px',borderRadius:'10px',cursor:'pointer',textAlign:'center',border:build.m===fid?`2px solid ${T.text}`:`1px solid ${T.border}`,background:build.m===fid?T.text+'10':T.cardAlt,position:'relative',display:'flex',flexDirection:'column',alignItems:'center',gap:'8px'}" @click="()=>{mid=fid;upBuild({m:fid,lamps:MD[fid].lamps})}">
                <div :style="{display:'flex',alignItems:'center',justifyContent:'center',width:'100%',gap:'6px'}"><span :style="{fontSize:'18px',fontWeight:800,color:build.m===fid?T.text:T.textSec,textAlign:'center',lineHeight:1.1}">{{ SIZE_WORD[MD[fid].letter] ?? MD[fid].letter }}</span><span v-if="recommendedMid===fid" :style="{width:'20px',height:'20px',borderRadius:'50%',background:T.text,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'12px',color:T.bg,fontWeight:700,flexShrink:0}">✓</span></div>
                <div v-if="hasRoomContext" :style="{padding:'6px 14px',borderRadius:'7px',background:T.text+'12',color:T.text,fontSize:'13px',fontWeight:700,whiteSpace:'nowrap'}">{{ brightLabel(fid) }}</div>
            ...
            <button v-if="recommendedMid&&build.m!==recommendedMid" :style="{width:'100%',padding:'12px',marginBottom:'14px',borderRadius:'10px',cursor:'pointer',border:`1px solid ${T.text}44`,background:T.text+'10',color:T.text,fontSize:'13px',fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',gap:'8px'}" @click="selectBestForMe"><span :style="{width:'20px',height:'20px',borderRadius:'50%',background:T.text,color:T.bg,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'12px',fontWeight:700}">✓</span>Выбери за меня</button>
          ...
          <div v-else-if="curStep==='wood'" :style="{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px'}"><button v-for="mt in simMats" :key="mt.id" :style="{padding:'16px 8px',borderRadius:'10px',cursor:'pointer',border:build.wood===mt.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.wood===mt.id?T.text+'15':T.card,textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center'}" @click="upBuild({wood:mt.id})"><div :style="{width:'40px',height:'40px',borderRadius:'50%',background:mt.color,border:mt.id==='black'?`2px solid ${T.textDim}`:'none'}"/><div :style="{fontSize:'14px',fontWeight:700,color:build.wood===mt.id?T.text:T.textSec,marginTop:'10px'}">{{ mt.name }}</div><div v-if="mt.tip" :style="{fontSize:'11px',color:T.textDim,marginTop:'6px',lineHeight:'1.4'}">{{ mt.tip }}</div></button></div>
          <div v-else-if="curStep==='mount'" :style="{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}"><button v-for="mt in SIM_MOUNTS" :key="mt.id" :style="{padding:'18px 12px',borderRadius:'8px',cursor:'pointer',textAlign:'center',border:build.mount===mt.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.mount===mt.id?T.text+'15':T.card,color:build.mount===mt.id?T.text:T.textSec}" @click="upBuild({mount:mt.id})"><div :style="{marginBottom:'8px',display:'flex',justifyContent:'center'}"><Icon :name="mt.id==='pendant'?'fxPendant':'fxFlush'" :size="28" :color="build.mount===mt.id?T.text:T.textDim"/></div><div :style="{fontSize:'13px',fontWeight:600}">{{ mt.name }}</div><div :style="{fontSize:'10px',color:T.textDim,marginTop:'6px'}">{{ mt.tip }}</div></button></div>
          <div v-else-if="curStep==='bowl'"><div v-if="freeBowls.length>0"><div :style="{fontSize:'10px',fontWeight:700,color:T.textDim,textTransform:'uppercase',letterSpacing:'.8px',marginBottom:'6px'}">{{ paidBowls.length>0?'Стандарт · входит в цену':'Выберите чашу' }}</div><div :style="{display:'grid',gridTemplateColumns:'repeat(auto-fill, minmax(100px, 1fr))',gap:'6px'}"><button v-for="b in freeBowls" :key="b.id" :style="{padding:'10px 8px',borderRadius:'8px',cursor:'pointer',textAlign:'center',border:build.bowl===b.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.bowl===b.id?T.text+'15':T.card,color:build.bowl===b.id?T.text:T.textSec}" @click="upBuild({bowl:b.id})"><div :style="{fontSize:'12px',fontWeight:600}">{{ b.name }}</div></button></div></div><div v-if="paidBowls.length>0" :style="{marginTop:freeBowls.length>0?'12px':'0'}"><div :style="{fontSize:'10px',fontWeight:700,color:T.textSec,textTransform:'uppercase',letterSpacing:'.8px',marginBottom:'6px'}">С доплатой</div><div :style="{display:'grid',gridTemplateColumns:'repeat(auto-fill, minmax(120px, 1fr))',gap:'6px'}"><button v-for="b in paidBowls" :key="b.id" :style="{padding:'10px 8px',borderRadius:'8px',cursor:'pointer',textAlign:'center',border:build.bowl===b.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.bowl===b.id?T.text+'15':T.card,color:build.bowl===b.id?T.text:T.textSec}" @click="upBuild({bowl:b.id})"><div :style="{fontSize:'12px',fontWeight:600}">{{ b.name }}</div><div :style="{fontSize:'11px',color:T.textSec,marginTop:'3px'}">+{{ fmt(b.price) }} ₽</div></button></div></div></div>
          <div v-else-if="curStep==='temp'" :style="{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'6px'}"><button v-for="bt in BTEMPS" :key="bt.id" :style="{padding:'12px 6px',borderRadius:'8px',cursor:'pointer',textAlign:'center',border:build.btemp===bt.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.btemp===bt.id?T.text+'15':T.card,color:build.btemp===bt.id?T.text:T.textSec}" @click="upBuild({btemp:bt.id})"><div :style="{fontSize:'13px',fontWeight:700}">{{ bt.label }}</div><div :style="{fontSize:'11px',color:T.textDim,marginTop:'2px'}">{{ bt.kelvin }}К</div><div v-if="bt.tip" :style="{fontSize:'10px',color:T.textSec,marginTop:'6px',lineHeight:'1.3'}">{{ bt.tip }}</div></button></div>
          ...
          <div v-else-if="curStep==='diffuser'" :style="{display:'flex',flexDirection:'column',gap:'8px'}"><button :style="{width:'100%',padding:'14px 16px',borderRadius:'10px',cursor:'pointer',textAlign:'left',border:build.diffuser?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.diffuser?T.text+'12':T.cardAlt,color:T.text,display:'flex',alignItems:'center',justifyContent:'space-between'}" @click="upBuild({diffuser:true})"><div><div :style="{fontSize:'14px',fontWeight:700}">Добавить рассеиватель</div><div :style="{fontSize:'11px',color:T.textDim,marginTop:'4px'}">{{ OPT_TIPS.diffuser.on }}</div></div><span :style="{fontSize:'14px',fontWeight:700,color:T.textSec,flexShrink:0}">+{{ fmt(OPT_PRICE.diffuser) }} ₽</span></button><button :style="{width:'100%',padding:'14px 16px',borderRadius:'10px',cursor:'pointer',textAlign:'left',border:!build.diffuser?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:!build.diffuser?T.text+'12':T.cardAlt,color:T.textSec,fontSize:'13px'}" @click="upBuild({diffuser:false})">{{ OPT_TIPS.diffuser.off }}</button></div>
          <div v-else-if="curStep==='wire'&&model.wireOpts" :style="{display:'flex',flexDirection:'column',gap:'6px'}"><button v-for="w in model.wireOpts" :key="w.id" :style="{textAlign:'left',display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 14px',borderRadius:'8px',cursor:'pointer',border:build.wire===w.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.wire===w.id?T.text+'15':T.card,color:build.wire===w.id?T.text:T.textSec}" @click="upBuild({wire:w.id})"><div><div :style="{fontSize:'13px',fontWeight:600}">{{ w.label }}</div><div v-if="w.tip" :style="{fontSize:'10px',color:T.textDim,marginTop:'2px'}">{{ w.tip }}</div></div><span v-if="w.price>0" :style="{fontSize:'12px',fontWeight:700,color:T.textSec,flexShrink:0}">+{{ fmt(w.price) }} ₽</span><span v-else :style="{fontSize:'10px',color:T.textSec,flexShrink:0}">стандарт</span></button></div>
          <div v-else-if="curStep==='base'" :style="{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}"><button v-for="(bc,bcId) in BASE_COLORS" :key="bcId" :style="{padding:'20px 10px',borderRadius:'8px',cursor:'pointer',textAlign:'center',border:build.baseColor===bcId?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.baseColor===bcId?T.text+'15':T.card,color:build.baseColor===bcId?T.text:T.textSec}" @click="upBuild({baseColor:String(bcId)})"><div :style="{width:'36px',height:'36px',borderRadius:'50%',background:bc.color,margin:'0 auto 8px',border:`1px solid ${T.border}`}"/><div :style="{fontSize:'14px',fontWeight:700}">{{ bc.name }}</div></button></div>
          <div v-else-if="curStep==='bulbs'&&model.bulbOpts" :style="{display:'flex',flexDirection:'column',gap:'6px'}"><div :style="{fontSize:'12px',color:T.textSec,marginBottom:'2px'}">Включить в комплект?</div><button v-for="bo in model.bulbOpts" :key="bo.id" :style="{textAlign:'left',display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px 16px',borderRadius:'8px',cursor:'pointer',border:build.bulbOpt===bo.id?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.bulbOpt===bo.id?T.text+'15':T.card,color:build.bulbOpt===bo.id?T.text:T.textSec,fontSize:'14px'}" @click="upBuild({bulbOpt:bo.id})"><span :style="{fontWeight:600}">{{ bo.label }}</span><span v-if="bo.price>0" :style="{fontWeight:700,color:T.textSec}">+{{ fmt(bo.price) }} ₽</span></button></div>
          <div v-else-if="curStep==='bulbs'" :style="{display:'flex',flexDirection:'column',gap:'8px'}"><button :style="{width:'100%',padding:'14px 16px',borderRadius:'10px',cursor:'pointer',textAlign:'left',border:build.bulbs?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:build.bulbs?T.text+'12':T.cardAlt,color:T.text,display:'flex',alignItems:'center',justifyContent:'space-between'}" @click="upBuild({bulbs:true})"><div><div :style="{fontSize:'14px',fontWeight:700}">Да, {{ build.lamps }} {{ slw(build.lamps) }} в комплекте</div><div :style="{fontSize:'11px',color:T.textDim,marginTop:'4px'}">{{ build.lamps }} {{ spw(build.lamps) }} × {{ fmt(bulbPer()) }} ₽</div></div><span :style="{fontSize:'14px',fontWeight:700,color:T.textSec,flexShrink:0}">+{{ fmt(bulbTotal()) }} ₽</span></button><button :style="{width:'100%',padding:'14px 16px',borderRadius:'10px',cursor:'pointer',textAlign:'left',border:!build.bulbs?(isTouched?'2px solid #fff':`2px solid ${T.text}`):`1px solid ${T.border}`,background:!build.bulbs?T.text+'12':T.cardAlt,color:T.textSec,fontSize:'13px'}" @click="upBuild({bulbs:false})">Подберу свои лампочки</button></div>
        </div>
