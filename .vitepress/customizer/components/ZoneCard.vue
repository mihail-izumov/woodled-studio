  ...
  >
    ...
    <div :style="{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', flexShrink: 0 }">
      ...
      <button
        ...
        :style="full
          ? { width: '34px', height: '34px', borderRadius: '50%', background: `linear-gradient(${accent}22, ${accent}22), ${T.card}`, border: `1px solid ${accent}66`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', padding: 0, flexShrink: 0 }
        ...
        @click.stop="onAdd"
      >
        <svg width="19" height="19" viewBox="0 0 24 24" fill="none" :stroke="full ? accent : '#F4ECDD'" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" :style="{ display: 'block', filter: full ? 'none' : 'drop-shadow(0 1px 1.5px rgba(0,0,0,0.5))' }"><path d="M5 12h14" /><path d="M12 5v14" /></svg>
      </button>
    </div>
