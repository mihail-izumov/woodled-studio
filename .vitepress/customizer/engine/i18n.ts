...
/**
 * Грамматический род типа светильника — для согласования глаголов/прилагательных
 * в прошедшем времени («удалён / удалена / удалено»).
 *   m — торшер, спот
 *   f — люстра, настольная
 *   n — бра
 */
export type FxGender = 'm' | 'f' | 'n'
export function fxGender(kind: FxType): FxGender {
  if (kind === 'люстра' || kind === 'настольная') return 'f'
  if (kind === 'бра') return 'n'
  return 'm'
}

/**
 * Окончание прошедшего времени по типу:
 *   м → '' («удалён»)
 *   ж → 'а' («удалена»)
 *   с → 'о' («удалено»)
 */
export function pastEnding(kind: FxType): string {
  const g = fxGender(kind)
  return g === 'f' ? 'а' : g === 'n' ? 'о' : ''
}
